...
class RealEstateApp {
    ...
    async checkServerConnection() {
        try {
            ...
            // Increased timeout to 8s to account for Vercel Cold Start
            ...
            if (resp.ok) {
                ...
                console.info('[RealEstateApp] Server connection verified:', data);
            } else {
                this.serverConnected = false;
                console.warn('[RealEstateApp] Server health check failed:', resp.status);
            }
        } catch (e) {
            this.serverConnected = false;
            // Don't log as error if just timeout, it might work on retry
            console.warn('[RealEstateApp] Server connection check:', e.message);
        } finally {
    ...
    async loadProperties() {
        // Preferred order: API -> localStorage -> fallback

        // ALWAYS try API first, even if health check timed out. 
        // The health check is just a hint, not a strict gatekeeper.
        try {
            console.log('[RealEstateApp] Attempting to fetch properties from API...');
            const resp = await this.fetchWithTimeout('/api/properties', {}, 15000);
            if (resp.ok) {
                const data = await resp.json();
                this.properties = data.properties || [];
                console.info('[RealEstateApp] Loaded properties from /api/properties:', this.properties.length);
                return;
            } else {
                console.warn('[RealEstateApp] API error:', resp.status);
            }
        } catch (e) {
            console.warn('[RealEstateApp] API request failed:', e.message);
        }

        // Then try localStorage (admin-created entries in this browser)
        const saved = localStorage.getItem('properties');
        if (saved) {
            try {
                this.properties = JSON.parse(saved);
                console.info('[RealEstateApp] Loaded properties from localStorage:', this.properties.length);
                return;
            } catch (e) {
                console.warn('[RealEstateApp] Failed to parse saved properties, continuing fallback:', e);
            }
        }

        // Finally, fallback to properties.json shipped with the site
        try {
            const response = await this.fetchWithTimeout('properties.json', {}, 3000);
            const data = await response.json();
            this.properties = data.properties || [];
            console.info('[RealEstateApp] Loaded properties from properties.json:', this.properties.length);
        } catch (error) {
            console.warn('[RealEstateApp] Failed to load properties.json:', error.message);
            // Fallback data
...
getFallbackProperties() {
    return [
        {
            id: 1,
            title: "Luxury Villa in Sangi-e-Masha",
            title_fa: "ویلای لوکس در سنگی مشا",
            type: "Villa",
            type_fa: "ویلا",
            price: 250000000000,
            price_fa: "۲۵۰۰ لک افغانی",
            area: 450,
            bedrooms: 5,
            bathrooms: 4,
            location: "Sangi-e-Masha, Jaghuri, Ghazni",
            location_fa: "سنگی مشا، جاغوری، غزنی",
            description: "Magnificent luxury villa in Sangi-e-Masha, Jaghuri. Features modern architecture and premium finishes.",
            description_fa: "ویلای لوکس باشکوه در سنگی مشا، جاغوری. دارای معماری مدرن و امکانات باکیفیت.",
            status: "Available",
            images: ["hero-villa.jpg"],
            features: ["Swimming Pool", "Private Garden", "Modern Kitchen", "Security System", "Garage"],
            features_fa: ["استخر", "باغ خصوصی", "آشپزخانه مدرن", "سیستم امنیتی", "گاراژ"],
            contact: "+93791606227",
...
setupHeroCarousel() {
    const heroImages = [
        'resources/Blue_Mosque_in_Mazar-e-Sharif.jpg',
        'resources/village-8492322.jpg',
        'resources/pato-8492323.jpg',
        'resources/pexels-azadphotos-11855445.jpg',
        'resources/pexels-azadphotos-11854377.jpg',
        'resources/pexels-azadphotos-12035242.jpg',
        'resources/pexels-azadphotos-32582537.jpg'
    ];
...
setupMap(property) {
    ...
    if (mapContainer && typeof google !== 'undefined') {
        const mapOptions = {
            ...
            zoom: 15,
            mapTypeId: 'roadmap', // Default to map view (Karte)
            streetViewControl: true,
            mapTypeControl: true
        };
