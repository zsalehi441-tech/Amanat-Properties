...
class RealEstateApp {
    ...
    async checkServerConnection() {
        try {
            ...
            if (e.name === 'AbortError') {
                console.warn('[RealEstateApp] Server connection timeout - server may not be running');
            } else {
                console.warn('[RealEstateApp] Server not available:', e.message);
            }
        } finally {
    ...
    async loadProperties() {
        // Preferred order: API -> localStorage -> local properties.json -> fallback
        try {
            // Try server API first if server is connected
            if (this.serverConnected) {
                try {
                    const resp = await this.fetchWithTimeout('/api/properties', {}, 5000);
                    if (resp.ok) {
                        const data = await resp.json();
                    ...
                    this.serverConnected = false; // Mark as disconnected on failure
                }
            } else {
                console.info('[RealEstateApp] Server not connected, skipping API call');
            }
